...
脚本会创建 `src/content/posts/my-first-post.md`，默认 `draft: true`、`lang: zh_CN`，发布日期填写运行当天。已有文件不会被覆盖。把标题、日期和正文改成自己的内容，浏览器中打开开发服务显示的地址。
...
访问地址为 `/posts/binary-search-notes/`。
...
目录中的 `index.md` 同样对应 `/posts/binary-search-notes/`。例如 `notes/binary-search.md` 对应 `/posts/notes/binary-search/`。不要同时保留 `binary-search-notes.md` 与 `binary-search-notes/index.md`，两者会得到同一个默认地址。
...
| `lang` | 文章语言，中文使用 `zh_CN`；英文文章可用 `en` |
...
对于 `src/content/posts/binary-search-notes/index.md`，将 `cover.png`、`steps.png` 放在同一个目录：
...
文章之间的链接使用页面地址，例如 `/posts/binary-search-notes/`，不要把本地 Markdown 文件路径当作线上链接。
